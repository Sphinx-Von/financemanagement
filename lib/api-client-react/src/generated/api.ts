...
export const getGetMonthlyTrendsUrl = (params?: GetMonthlyTrendsParams) => {
  ...
  return stringifiedParams.length > 0
    ? `/api/dashboard/monthly-trends?${stringifiedParams}`
...
export type GetMonthlyTrendsQueryResult = NonNullable<
  Awaited<ReturnType<typeof getMonthlyTrends>>
>;
export type GetMonthlyTrendsQueryError = ErrorType<ErrorResponse>;
 ...
 */
export const getCreateRentalSetupUrl = () => {
  return `/api/rental/setup`;
};

export const createRentalSetup = async (
  createRentalSetupBody: CreateRentalSetupBody,
  options?: RequestInit,
): Promise<MessageResponse> => {
  return customFetch<MessageResponse>(getCreateRentalSetupUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(createRentalSetupBody),
  });
};

export const getCreateRentalSetupMutationOptions = <
  TError = ErrorType<ErrorResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof createRentalSetup>>,
    TError,
    { data: BodyType<CreateRentalSetupBody> },
    TContext
  >;
  request?: SecondParameter<typeof customFetch>;
}): UseMutationOptions<
  ...
  TContext
> => {
  const mutationKey = ["createRentalSetup"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return createRentalSetup(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type CreateRentalSetupMutationResult = NonNullable<
  Awaited<ReturnType<typeof createRentalSetup>>
>;
export type CreateRentalSetupMutationBody = BodyType<CreateRentalSetupBody>;
export type CreateRentalSetupMutationError = ErrorType<ErrorResponse>;

/**
 * @summary Save rental property and payment details
 */
export const useCreateRentalSetup = <
  TError = ErrorType<ErrorResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getCreateRentalSetupMutationOptions(options));
};
